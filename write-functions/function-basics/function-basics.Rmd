...
* Use `environment()` to see the environment associated with the `pivot_longer` function.
...
environment(pivot_longer)
```

```{r gather-env-check}
"The pivot_longer() function comes in the tidyr package. If a function's environment is namespace:base, that is a good sign that the function comes in base R, the part of R that loads before you load any optional packages."
```
